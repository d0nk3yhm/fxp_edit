...
def generate_fxp(plugin, json_file_path, fxp_output_path):
    with open(json_file_path, 'r') as json_file:
        data = json.load(json_file)

    # Apply updated parameters to the plugin
    for index, value in data["parameters"].items():
        try:
    ...
    # Generate new chunk data from the plugin
    new_chunk_data = plugin.get_chunk()

    # Create FXP file with updated chunk data
...
def api_endpoint():
    if request.is_json:
        ...
        generate_fxp(plugin, json_file_path, modified_fxp_file_path)
